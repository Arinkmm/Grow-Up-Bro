...
- Внедрено принудительное приведение типов (валидация Boolean-флагов из CSV-строк: item['is_verified'].lower() in ['true', '1', 't'], приведение рейтинга к числу: row['rating'] = int(row['rating']) if row['rating'] else 0)
